...
function Brain(refTable, numAngles, numSpeeds, speed){
	...
	this.findShotAngs = function(){
		this.shotAngs = [];

		for (var i = 0; i < this.numAngles * this.numSpeeds; i++) {
			let castAng = new LineSegment();
		}
	}
	...
	this.shoot = function(){
		if(this.shots.length >= this.numShots){
			...
			this.refTable.shotMeter.shotPower = this.shots[0].p;
			this.refTable.shotMeter.pullBack = 500;
			this.refTable.shotAng = this.shots[0].ang;
...
function getScoreFromTable(refTable, ang, colorToPick){
	...
	for (var j = 0; j < refTable.pockets.length; j++) {
		for (var i = 0; i < refTable.balls.length; i++) {
			...
			if(refTable.balls[i].color == "white"){
				score -=  1 / 2*distanceOfTwoPoints(refTable.pockets[j].pos.x, refTable.pockets[j].pos.y,
												  refTable.balls[i].pos.x, refTable.balls[i].pos.y);
			}
